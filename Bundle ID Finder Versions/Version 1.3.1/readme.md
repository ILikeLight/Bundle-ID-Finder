...
.DESCRIPTION
    ...
    and checks for script updates from GitHub.
.AUTHOR
    ILikeLight
.VERSION
    1.3.1
...
function Check-ScriptUpdate {
    try {
        ...
        if ($needsUpdate -or $latestParts.Length -gt $currentParts.Length) {
            ...
            Write-Host "Download Link:   " -NoNewline
            ...
            Write-Host "✅ You are running the latest version ($CURRENT_VERSION)" -ForegroundColor Green
            return
        }
    }
    catch {
